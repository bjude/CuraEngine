...
void FffPolygonGenerator::processDerivedWallsSkinInfill(SliceMeshStorage& mesh)
{
    ...
    else
    {
        ...
        if (mesh.getSettingInMicrons("infill_line_distance") > 0
            && mesh.getSettingAsCount("bottom_layers") < mesh.bounding_box.size().z / mesh.getSettingInMicrons("layer_height")
        ...
        )
        {
            ...
            if (cross_subdisivion_spec_image_file != "" && cross_fs.good())
            {
                ...
                mesh.cross_fill_provider = new SierpinskiFillProvider(mesh.bounding_box, mesh.getSettingInMicrons("infill_line_distance"), mesh.getSettingInMicrons("infill_line_width"), cross_subdisivion_spec_image_file, use_new_cross_3d_generator);
            }
            else
            {
                ...
                mesh.cross_fill_provider = new SierpinskiFillProvider(mesh.bounding_box, mesh.getSettingInMicrons("infill_line_distance"), mesh.getSettingInMicrons("infill_line_width"), mesh.getSettingInPercentage("cross_infill_dithering_density"));
            }
        }
